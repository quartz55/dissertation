...
def build_targets(pred_boxes, target, anchors, num_anchors, num_classes, nH, nW, noobject_scale, object_scale,
    ...
    if seen < 12800:
        if anchor_step == 4:
            tx = anchors.view(nA, anchor_step).index_select(1, torch.LongTensor([2])).view(1, nA, 1, 1).repeat(nB, 1,
                                                                                                               nH, nW)
            ty = anchors.view(num_anchors, anchor_step).index_select(1, torch.LongTensor([2])).view(1, nA, 1, 1).repeat(
                nB, 1, nH, nW)
        else:
            tx.fill_(0.5)
            ty.fill_(0.5)
    ...
    for b in range(nB):
        for t in range(50):
            ...
            iou = utils.bbox_iou(gt_box, pred_box, x1y1x2y2=False)  # best_iou
...
class YoloLayer(nn.Module):
    def __init__(self, anchor_mask: List[int] = None, num_classes=0, anchors=None, num_anchors=1):
        super(YoloLayer, self).__init__()
        self.anchor_mask = torch.IntTensor(anchor_mask if anchor_mask is not None else [])
        self.num_classes = num_classes
        self.anchors = torch.FloatTensor(anchors if anchors is not None else [])
    ...
    def forward(self, output, target=None):
        if self.training:
            ...
            grid_y = torch.linspace(0, nH - 1, nH).repeat(nW, 1).t().repeat(nB * nA, 1, 1).view(
                nB * nA * nH * nW).cuda()
            anchor_w = self.anchors.view(nA, self.anchor_step).index_select(1, torch.LongTensor([0])).cuda()
            anchor_h = self.anchors.view(nA, self.anchor_step).index_select(1, torch.LongTensor([1])).cuda()
            ...
            if self._logger.isEnabledFor(logging.DEBUG):
                ...
                debug('             total : %f' % (t4 - t0))
                debug('%d: nGT %d, recall %d, proposals %d, loss: x %f, y %f, w %f, h %f, conf %f, cls %f, total %f' % (
                    self.seen, nGT, nCorrect, nProposals, loss_x.data[0], loss_y.data[0], loss_w.data[0],
                    loss_h.data[0],
